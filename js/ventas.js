...
function renderModalFin(v) {
  ...
  document.getElementById('fin-resumen').innerHTML = `
    <div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);padding:14px;margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:10px">
        <div><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.5px">Producto</div><div style="font-weight:600">${v.producto}</div></div>
        <div><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.5px">Precio total</div><div style="font-weight:700;font-family:var(--mono)">${fmt(v.precio)}</div></div>
        <div><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.5px">Abonado</div><div style="font-weight:700;font-family:var(--mono);color:var(--green)">${fmt(ab)}</div></div>
        <div><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.5px">Saldo</div><div style="font-weight:700;font-family:var(--mono);color:${sal > 0 ? 'var(--amber)' : 'var(--green)'}">${fmt(sal)}</div></div>
      </div>
  ...
  document.getElementById('fin-tabs').innerHTML = `
    <div style="display:flex;gap:0;border-bottom:1px solid var(--border);margin-bottom:16px">
      ...
      <button class="fin-tab" id="tab-plan" onclick="showFinTab('plan')">${tieneplan ? '📋 Ver plan de cuotas' : '🗓 Crear plan de cuotas'}</button>
    </div>`;

  let abonoHtml = sal <= 0
    ? `<div class="alert success">✓ Esta venta está completamente pagada.</div>`
    : `<div class="form-row" style="margin-bottom:12px">
        <div class="form-group" style="margin:0"><label>Monto del abono (COP $)</label><input type="number" id="fin-av-monto" placeholder="0"></div>
        <div class="form-group" style="margin:0"><label>Observación</label><input id="fin-av-obs" placeholder="Ej: Cuota 1, pago parcial..."></div>
      </div>
      <button class="btn primary" onclick="registrarAbonoLibre()" style="width:100%;justify-content:center">+ Registrar abono</button>`;

  if (misAbonos.length) {
    abonoHtml += `<div style="margin-top:16px;border-top:1px solid var(--border);padding-top:12px">
      <div style="font-size:11px;font-weight:600;color:var(--text3);letter-spacing:.5px;text-transform:uppercase;margin-bottom:8px">Historial de abonos</div>
      ${misAbonos.map(a => `<div class="abono-item">
        <div><strong style="font-family:var(--mono);color:var(--green)">${fmt(a.monto)}</strong>${a.obs ? `<span style="color:var(--text3)"> · ${a.obs}</span>` : ''}</div>
  ...
  if (tieneplan) {
    ...
    ${misCuotas.map(c => {
      ...
      return `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:var(--bg3);border-radius:var(--radius-sm);margin-bottom:6px;border:1px solid var(--border)">
    ...
    planHtml = `<div class="alert info" style="margin-bottom:16px">📅 Crea un plan de cuotas fijas con fechas de vencimiento.</div>
      <div class="form-row">
        <div class="form-group"><label>Número de cuotas</label>
          <select id="fin-ncuotas" onchange="previewPlan()">
            <option value="2">2 cuotas</option><option value="3" selected>3 cuotas</option>
            <option value="4">4 cuotas</option><option value="5">5 cuotas</option><option value="6">6 cuotas</option>
          </select>
        </div>
        <div class="form-group"><label>Tasa mensual (%)</label><input type="number" id="fin-tasa-plan" value="3.5" step="0.1" oninput="previewPlan()"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>Cuota inicial (COP $)</label><input type="number" id="fin-ini-plan" placeholder="0" oninput="previewPlan()"></div>
        <div class="form-group"><label>Fecha primer vencimiento</label><input type="date" id="fin-fecha-plan" value="${nextMonth()}" oninput="previewPlan()"></div>
      </div>
      <div id="fin-plan-preview"></div>
      <button class="btn primary" onclick="crearPlan()" style="width:100%;justify-content:center;margin-top:12px">📋 Crear plan de cuotas</button>`;
  }
...
function previewPlan() {
  ...
  let rows = '';
  for (let i = 0; i < nc; i++) {
    rows += `<tr><td>Cuota ${i+1}</td><td style="font-family:var(--mono);color:var(--green);font-weight:600">${fmt(Math.round(cuota))}</td><td style="font-size:12px;color:var(--text2)">${addMonths(fecha, i)}</td></tr>`;
  }
  el.innerHTML = `<div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;margin-top:12px">
    ...
    <table style="min-width:unset"><thead><tr><th>Cuota</th><th>Monto</th><th>Vencimiento</th></tr></thead><tbody>${rows}</tbody></table>
  </div>`;
}
...
async function crearPlan() {
  ...
  try {
    ...
    v.estado = 'Financiada'; v.cuotas = nc;
    if (ini > 0) {
      const [a] = await sb('abonos', 'POST', { tipo: 'venta', ref_id: finVentaId, monto: ini, obs: 'Cuota inicial', fecha: today() });
      abonos.push(a);
    }
    for (let i = 0; i < nc; i++) {
      const [c] = await sb('cuotas', 'POST', { venta_id: finVentaId, numero: i+1, monto: Math.round(cuota), fecha_venc: addMonths(fecha, i), estado: 'Pendiente' });
      cuotas.push(c);
    }
    renderModalFin(ventas.find(x => x.id === finVentaId));
...
async function marcarCuotaPagada(cuotaId) {
  ...
  try {
    ...
    c.estado = 'Pagada'; c.fecha_pago = today();
    const [a] = await sb('abonos', 'POST', { tipo: 'venta', ref_id: finVentaId, monto: c.monto, obs: `Cuota ${c.numero} pagada`, fecha: today() });
    abonos.push(a);
    ...
    if (pendientes.length === 0) {
      ...
      if (v) v.estado = 'Completada';
      toast('¡Venta completamente pagada! 🎉');
    } else { toast('Cuota marcada como pagada ✓'); }
    renderModalFin(ventas.find(x => x.id === finVentaId));
    renderVentas(); renderDashboard();
  } catch (e) { toast('Error: ' + e.message, 'err'); }
}
...
async function eliminarPlan() {
  ...
  try {
    ...
    const v = ventas.find(x => x.id === finVentaId);
    if (v) { v.cuotas = 0; await sb('ventas', 'PATCH', { cuotas: 0 }, `?id=eq.${finVentaId}`); }
    renderModalFin(ventas.find(x => x.id === finVentaId));
    renderVentas();
...
function abrirModalFechaPlan(ventaId, meses, cuotaMonto, financiado, inicial) {
  ...
  document.getElementById('fecha-plan-resumen').innerHTML = `
    <div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);padding:12px">
      <div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="color:var(--text3);font-size:12px">Total financiado</span><span style="font-family:var(--mono);font-weight:600">${fmt(financiado)}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="color:var(--text3);font-size:12px">Inicial pagada</span><span style="font-family:var(--mono);color:var(--amber)">${fmt(inicial)}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="color:var(--text3);font-size:12px">Cuota mensual</span><span style="font-family:var(--mono);color:var(--green);font-weight:700">${fmt(Math.round(cuotaMonto))}</span></div>
      <div style="display:flex;justify-content:space-between"><span style="color:var(--text3);font-size:12px">Número de cuotas</span><span style="font-weight:600">${meses} meses</span></div>
    </div>`;
...
async function crearPlanAuto() {
  ...
  try {
    ...
    if (v) {
      ...
      v.estado = 'Financiada'; v.cuotas = meses;
    }
    for (let i = 0; i < meses; i++) {
      const [c] = await sb('cuotas', 'POST', { venta_id: ventaId, numero: i+1, monto: Math.round(cuotaMonto), fecha_venc: addMonths(fecha, i), estado: 'Pendiente' });
      cuotas.push(c);
    }
...
function renderAbonoLista(tipo, id, elId) {
  ...
  if (!list.length) { el.innerHTML = '<p style="color:var(--text3);font-size:12px;margin-top:8px">No hay abonos registrados aún.</p>'; return; }
  el.innerHTML = `<div style="margin-top:10px;border-top:1px solid var(--border);padding-top:10px">
    ...
    ${list.map(a => `<div class="abono-item">
      <div><strong style="font-family:var(--mono);color:var(--green)">${fmt(a.monto)}</strong>${a.obs ? ` <span style="color:var(--text3)">· ${a.obs}</span>` : ''}</div>
      <div class="abono-fecha">${a.fecha}</div>
    </div>`).join('')}
  </div>`;
