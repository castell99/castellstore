...
async function dibujarRecibo(datos, tipo, refId) {
  ...
  if (datos.diagnostico) {
    ctx.fillStyle = '#8a9aa1';
    ctx.font      = '11px Outfit, sans-serif';
    ctx.fillText('DIAGNÓSTICO / SERVICIO', 60, y + 108);
    ctx.fillStyle = '#e8f0f5';
    ctx.font      = '12px Outfit, sans-serif';
...
function mostrarVistaPrevia(dataURL, datos, tipo, refId) {
  ...
  // Auto-descargar la imagen inmediatamente
  const link  = document.createElement('a');
  link.href   = dataURL;
  link.download = nombreArchivo;
  link.click();

  // Mensaje de WhatsApp (solo texto — la imagen se adjunta manualmente)
  const msg = encodeURIComponent(
    ...
    `Le compartimos la imagen del recibo de paz y salvo adjunta a este mensaje.\n\n` +
    `¡Gracias por su confianza! 🙌\n` +
    `📞 ${NEGOCIO.telefono} | ${NEGOCIO.nombre}`
  );
  const waURL = `https://wa.me/?text=${msg}`;

  // Modal de confirmación con pasos visuales
  ...
  m.innerHTML = `
    <div class="modal" style="max-width:660px">
      ...
      <div style="text-align:center;margin-bottom:16px">
        <img src="${dataURL}"
          style="width:100%;border-radius:var(--radius);border:1px solid var(--border);cursor:zoom-in"
          alt="Paz y Salvo"
      ...
      <div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);padding:14px;margin-bottom:14px">
        <div style="font-size:11px;font-weight:600;color:var(--text3);letter-spacing:.5px;text-transform:uppercase;margin-bottom:10px">
        ...
        <div style="display:flex;gap:0;align-items:stretch">
          ...
          <div style="flex:1;text-align:center;padding:10px 8px">
            ...
            <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:3px">Paso 1</div>
            <div style="font-size:11px;color:var(--text2)">La imagen ya se descargó en tu dispositivo automáticamente</div>
            <div style="margin-top:8px">
              <a href="${dataURL}" download="${nombreArchivo}">
                <button class="btn sm" style="font-size:11px">Descargar de nuevo</button>
              </a>
            </div>
          </div>
          ...
          <div style="flex:1;text-align:center;padding:10px 8px">
            ...
            <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:3px">Paso 2</div>
            <div style="font-size:11px;color:var(--text2)">Abre WhatsApp con el mensaje listo. Solo adjunta la imagen descargada</div>
            <div style="margin-top:8px">
              <a href="${waURL}" target="_blank">
                <button class="btn sm primary" style="font-size:11px;background:#25D366;border-color:#25D366">
                  Abrir WhatsApp
                </button>
              </a>
            </div>
          ...
          <div style="flex:1;text-align:center;padding:10px 8px">
            <div style="font-size:24px;margin-bottom:6px">📎</div>
            <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:3px">Paso 3</div>
            <div style="font-size:11px;color:var(--text2)">En WhatsApp toca el clip 📎, selecciona la imagen del paz y salvo y envía</div>
          </div>
      ...
      <div class="alert info" style="font-size:12px">
        💡 <strong>En celular:</strong> la imagen se guarda en tu galería. Desde WhatsApp toca 📎 → Galería → selecciona la imagen.
        <strong>En PC:</strong> se descarga en tu carpeta de Descargas.
      </div>

      <div class="modal-footer">
        <button class="btn" onclick="document.getElementById('modal-recibo').classList.remove('open')">Cerrar</button>
        <a href="${dataURL}" download="${nombreArchivo}">
          <button class="btn">⬇️ Descargar imagen</button>
        </a>
        <a href="${waURL}" target="_blank">
          <button class="btn" style="background:#25D366;border-color:#25D366;color:#fff">
